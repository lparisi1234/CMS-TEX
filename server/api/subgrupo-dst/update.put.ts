...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (destino_id === undefined || destino_id === null) {
      return { success: false, message: 'ID de destino es requerido' }
    }
    ...
    try {
      ...
      const subgrupoValues = [nombre, destino_id, nro_orden, segmentosArray, id]
      ...
      if (productos_ids && productos_ids.length > 0) {
        ...
        for (const codigo_completo of productos_ids) {
          ...
          await client.query(createRelacionQuery, [producto.id, null, id])
